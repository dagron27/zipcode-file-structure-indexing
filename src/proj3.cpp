/** 
 * @file proj3.cpp
 * @brief A program to analyze geographical data from a CSV file and display extreme zip codes for each state.
 * 
 * This program reads geographical data from a CSV file, processes it to find extreme zip codes (easternmost, westernmost, northernmost, and southernmost) for each state,
...
// Function prototypes
void processData(Buffer& buffer, std::map<std::string, double>& easternmost, std::map<std::string, double>& westernmost, std::map<std::string, double>& northernmost, std::map<std::string, double>& southernmost);
...
int main() {
    ...
    std::cout << "Enter the name of the CSV file to be opened: ";
    std::getline(std::cin, inputFileName);

    // Create a Buffer object for your CSV file
    ...
    // Process the data
    processData(buffer, easternmost, westernmost, northernmost, southernmost);
